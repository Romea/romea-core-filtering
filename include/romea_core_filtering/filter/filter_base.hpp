...
class FilterBase {
 public:
  ...
  using UpdateFunction =
      typename FilterMetaState<State, FSMState, Duration>::UpdateFunction;
...
//-----------------------------------------------------------------------------
template <class State, class FSMState, class Duration>
void FilterBase<State, FSMState, Duration>::register_predictor(
    PredictorPtr predicter) {
...
bool FilterBase<State, FSMState, Duration>::get_current_state(
    Duration const& currentDuration, State* current_state) {
  ...
  assert(std::distance(std::begin(meta_states_), I) <
         ptrdiff_t(meta_states_.size()));
  assert((*I).state.get());
  ...
  predictor_->predict(previous_duration, previous_fsm_state, *previous_state,
                      currentDuration, current_fsm_State, *current_state);
...
void FilterBase<State, FSMState, Duration>::process(
  ...
  if (!meta_states_.empty()) {
    ...
    if (Ir == meta_states_.rend() || duration < meta_states_[0].duration) {
      std::cout << " Discard metaState because is prior to the first metaState"
                << std::endl;
      return;
    }
  ...
  if (meta_states_.size() < state_vector_pool_.size()) {
    ...
    I = meta_states_.emplace(I, duration, std::move(state),
  ...
  while (I != meta_states_.end()) {
    ...
    if (I != std::begin(meta_states_)) {
      ...
      predictor_->predict(previous_duration, previous_fsm_state,
                          *previous_state, currentDuration, current_fsm_State,
                          *current_state);
