...
class PFGaussianUpdaterBase : public PFUpdaterBase<Scalar, StateDIM, ObservationDIM>
{
public:
...
protected:
  ...
  virtual void compute_innovation_(const Observation & observation, const RawMajorVector & weights);
...
//-----------------------------------------------------------------------------
template<typename Scalar, size_t StateDIM, size_t ObservationDIM>
void PFGaussianUpdaterBase<Scalar, StateDIM, ObservationDIM>::compute_innovation_(
  const Observation & observation, const RawMajorVector & weights)
{
  Scalar weight_sum = weights.sum();

  for (size_t i = 0; i < ObservationDIM; ++i) {
    apriori_observation_.Y(i) = (apriori_observations_.row(i) * weights).sum() / weight_sum;
...
class PFGaussianUpdaterBase<Scalar, StateDIM, 1> : public PFUpdaterBase<Scalar, StateDIM, 1>
{
public:
  using State = ParticleFilterState<Scalar, StateDIM>;
...
protected:
  ...
  void compute_innovation_(const Observation & observation, const WeightVector & weights);
...
{
  compute_innovation_(observation, state.weights);
  mahalanobis_distance_ = KFMahalanobis<Scalar, 1>::compute(Inn_, QInn_, QInnInverse_);
  if (mahalanobis_distance_ < maximal_mahalanobis_distance_) {
    state.weights *=
