#ifndef romea_PFGaussianUpdaterCore_hpp
#define romea_PFGaussianUpdaterCore_hpp

//romea
#include "ParticleFilterUpdaterCore.hpp"
#include "../GaussianObservation.hpp"
#include "../kalman/core/KFMahalanobis.hpp"
...
public :
  ...
  PFGaussianUpdaterCore(const std::size_t & numberOfParticles,
...
//-----------------------------------------------------------------------------
template <typename Scalar, size_t StateDIM, size_t ObservationDIM>
PFGaussianUpdaterCore<Scalar,StateDIM,ObservationDIM>::PFGaussianUpdaterCore(const std::size_t &numberOfParticles,
                                                                             const double & maximalMahalanobisDistance):
...
PFGaussianUpdaterCore<Scalar,StateDIM,ObservationDIM>::computeInnovation_(const Observation & observation,
                                                                          const RawMajorVector &weights)
{
  ...
  for(size_t i=0 ; i<ObservationDIM ; ++i){
    for(size_t j=i ; j<ObservationDIM ; ++j){
      aprioriObservation_.R(i,j) = aprioriObservation_.R(j,i)= (aprioriMeanCenteredObservations_.row(i)*
...
//-----------------------------------------------------------------------------
template <typename Scalar, size_t StateDIM, size_t ObservationDIM>
bool PFGaussianUpdaterCore<Scalar,StateDIM,ObservationDIM>::updateState_(State & state,const Observation & observation)
{
...
//-----------------------------------------------------------------------------
template <typename Scalar, size_t StateDIM>
PFGaussianUpdaterCore<Scalar,StateDIM,1>::PFGaussianUpdaterCore(const std::size_t &numberOfParticles,
                                                                const double & maximalMahalanobisDistance):
...
//-----------------------------------------------------------------------------
template <typename Scalar, size_t StateDIM>
void PFGaussianUpdaterCore<Scalar,StateDIM,1>::computeInnovation_(const Observation & observation,
                                                                  const WeightVector & weights)
{
  aprioriObservation_.Y()=(aprioriObservations_*weights).sum();
  aprioriObservation_.R()=((aprioriObservations_-aprioriObservation_.Y())*weights).square().sum();
  this->Inn_= observation.Y() - aprioriObservation_.Y();
  this->QInn_=observation.R() + aprioriObservation_.R();
}
...
//-----------------------------------------------------------------------------
template <typename Scalar, size_t StateDIM>
bool PFGaussianUpdaterCore<Scalar,StateDIM,1>::updateState_(State & state,const Observation & observation)
{
  computeInnovation_(observation,state.weights);
