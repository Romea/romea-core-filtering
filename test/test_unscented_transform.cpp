...
ObservationSigmaPoints make_reference_propagated_sigma_points() {
  return {11.07597392, 11.1579882, 11.10114414, 11.09709313,
          10.99435493, 11.05318411, 11.05509837};
}

void expect_vector_near(const Eigen::Vector3d& value,
...
TEST(TestUnscentedTransform, convertsGaussianStateToSigmaPoints) {
  ...
  expect_vector_near(sigma_points[0],
                     Eigen::Vector3d(10.04011967, -6.541962718, 5.773763097),
                     0.01);
  expect_vector_near(sigma_points[1],
                     Eigen::Vector3d(10.13285102, -6.489773659, 5.808572647),
                     0.01);
  expect_vector_near(sigma_points[2],
                     Eigen::Vector3d(10.09230873, -6.385416757, 5.833716403),
                     0.01);
  expect_vector_near(sigma_points[3],
                     Eigen::Vector3d(10.07492922, -6.482009413, 5.896015634),
                     0.01);
  expect_vector_near(sigma_points[4],
                     Eigen::Vector3d(9.947388315, -6.594151777, 5.738953548),
                     0.01);
  expect_vector_near(sigma_points[5],
                     Eigen::Vector3d(9.98793061, -6.698508679, 5.713809792),
                     0.01);
  expect_vector_near(sigma_points[6],
                     Eigen::Vector3d(10.00531012, -6.601916023, 5.651510561),
                     0.01);
}
...
TEST(TestUnscentedTransform, reconstructsGaussianObservationFromSigmaPoints) {
  ...
  romea::core::UnscentedTransformInverse<double, 1>::to_gaussian(
      make_reference_parameters(), make_reference_propagated_sigma_points(),
      propagated_state);
...
TEST(TestUnscentedTransform, computesStateObservationCorrelation) {
  ...
  romea::core::UKFCorrelation<double, 3, 1>::compute(
      make_reference_parameters(), make_reference_state(), propagated_state,
      make_reference_sigma_points(), propagated_sigma_points,
